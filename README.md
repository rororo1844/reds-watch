...
取得失敗は実行ログにだけ記録され、Discord には通知しない。
ログで全インスタンスの失敗を確認したら、生きているものを探して `NITTER_INSTANCES` を差し替える。
...
Nitter が全滅して取得に失敗しても、停止通知および復旧通知は Discord に送信しない。
失敗は Cloudflare Workers のエラーとログに記録される。
...
| 2 | 変化なし | KV書込0回 |
| 3 | 全インスタンス失敗 | エラーのみ。Discord通知なし |
| 4-5 | 障害継続 | エラーのみ。Discord通知なし |
| 6 | 復旧 | 通常運転に戻る。復旧通知なし |
| 7 | 平常 | 何も起きない |
